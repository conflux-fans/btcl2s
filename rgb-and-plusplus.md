...
- [RGB++](#rgb-1)
  ...
  - [更多细节](#更多细节-1)
    - [同构绑定](#同构绑定)
    - [global state 带来的安全风险](#global-state-带来的安全风险)
  - [相关资源](#相关资源-1)
...
而因为UTXO只能被花费一次，所以与该UTXO绑定的RGB资产也只能被转移一次。如果比特币安全，UTXO无法被双花，RGB资产也无法被双花，从而保障了协议安全。RGB将这一设计称作[一次性封装](https://blackpaper.rgb.tech/consensus-layer/3.-client-side-validation/3.2.-single-use-seals)。
...
RGB这种链下计算引擎做的事情在于仅利用比特币的共识来维护交易顺序，从而避免双花。状态、状态转换则在链下维护。这种范式的优点在于将执行层与共识层进行了解耦，使得不升级区块链便能扩展功能。但共识层在执行时不会进行逻辑验证，使得服务的可用性下降。某种程度我们也可以认为，RGB是“寄生”在了比特币上。
...
这里也是RGB协议需要交互的另一个核心原因（需要参与各方签名）。感兴趣的读者可以阅读RGB官方文档的[合约使用](https://rgb.tech/power-user/)部分，了解相关参与方交互的流程。
...
用户不必信任CKB的安全性（safety，或者认为是防止双花的能力），这是因为对应比特币上的承诺，恶意的攻击者无法构造出另一笔CKB交易，包含不同的执行内容。

需要注意的是，上面的信任分析没有考虑RGB++计划引入的扩展功能，例如交易折叠、共享状态等功能。
...
这一点其实带来了潜在的安全问题。这一问题来自于RGB的proof-of-publication设计。该设计使得只保证单一路径上状态转换的偏序，无需保证全部状态转换的全序，就能保证协议安全。
