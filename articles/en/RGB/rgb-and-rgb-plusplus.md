...
> In this section, we'll focus on the essentials and keep the technical jargon to a minimum, making the protocol's workings easy to understand.
...
RGB's design as a smart contract platform is intricately linked with UTXOs. To illustrate, imagine 100 units of an asset being issued on the RGB platform. These assets are tied to a specific UTXO. When an authorized user spends that UTXO, they can dictate the flow of RGB assets within the transaction (a concept known as **ownership**). The diagram below provides a visual:
...
#### Indicating RGB Actions on the Blockchain

To maintain privacy and enhance the capabilities of smart contracts, RGB doesn't etch operations in plain text on the Bitcoin script. Instead, it submits a hash value related to the operation to the blockchain (referred to as a "commitment"). The RGB protocol's participants can then validate whether the commitment on the blockchain matches the intended operation off the chain.

> We won't delve into the mechanics of how commitments are submitted. However, it's important to note that a corresponding commitment value will always be present on the blockchain.
...
As the hash value cannot be reversed to reveal the original RGB operation, those not involved with RGB remain unaware of the assets' existence.
...
Using the example from the [Basic Operation](#basic-operation) section, Bob, the owner of UTXO1, wishes to transfer 70 RGB assets to Alice, who holds UTXO4.
...
> 
> Unless stated otherwise, we assume that the classical cryptographic algorithms used within the protocol are reliable, disregarding the potential impact of quantum computing.

- Users must have confidence in the Bitcoin network.
...
- Users should trust the off-chain execution mechanism of RGB, which includes the virtual machine and the design of smart contracts. RGB assets are linked to UTXOs, and while only hash values of RGB operations are submitted to the blockchain, there is no direct interaction with Bitcoin scripts. Consequently, the blockchain cannot confirm the validity of RGB transactions in any way. If the off-chain execution mechanism encounters an error (such as a coding mistake in the engine or a flaw in the contract logic), it could result in the creation of invalid states off-chain or a discrepancy between the submitted hashes and the actual RGB status. In such cases, once the UTXO transaction is spent, the state in the user's possession may not be verifiable, potentially leading to a loss of assets.

The RGB protocol minimizes the trust required from users. Users can independently verify nearly every aspect of the protocol to ensure its proper functioning. However, this also imposes certain responsibilities on the users.
...
#### Consensus and the Role of Off-Chain Computing
...
1. Each copy begins with a specific state as the starting point.
2. State transitions occur as follows:
   1. Before receiving new input, the state of each copy remains the same.
...
Hashing is a frequently used technique for creating commitment schemes. For example, the Merkle Tree, a well-known concept in blockchain technology, is a type of commitment scheme. Commitments are also commonly used in other blockchain applications, such as the two-stage registration process in ENS, which helps prevent others from preemptively registering domain names.

#### Proof of Publication

A potential concern in RGB's client-side validation is that transaction participants may not have access to the overall state of assets. For example, while Bob may hold 100 assets, there could be 200 assets in circulation. How does the system ensure that contract execution does not lead to issues under such circumstances?
...
In the example, Bob is transferring 70 RGB assets to Alice. Unlike the previous method, all RGB-related data is now publicly available on the CKB chain, with CKB taking on the role of the transaction engine to manage and maintain the RGB contract state.
...
- Once the Bitcoin transaction (0x345AB...D7654) that includes the commitment is confirmed, Bob can submit a transaction with the RGB data (data2) to CKB. CKB nodes, running a lightweight version of the Bitcoin node, will verify the existence of the commitment on the Bitcoin network.
...
### Details for Translation Workflow
...
- Off-chain Calculation
  - Bob chooses the next UTXO (Unspent Transaction Output) to be used for the BTC transaction, such as UTXO1.
  - Bob performs calculations off the blockchain to create an RGB++ transaction for submission to the CKB network: CKB_TX. This transaction includes the necessary data for changes in the RGB state.
...
It should be noted that the above trust analysis does not take into account the extended features planned by the RGB++ project, such as transaction folding and global state functions.

### RGB++ Summary: Advantages and Disadvantages

RGB++ is an advanced version of RGB, let's analyze the strengths and weaknesses of RGB++ in comparison to its predecessor.
...
- There is an introduction of reliance on the CKB chain.
...
The CKB's ability to act as the RGB computation engine is greatly enhanced by the isomorphic nature of its Cells to Bitcoin's UTXOs. This allows CKB to adopt many designs from the RGB protocol, such as the proof-of-publication mechanism, which helps to prevent state contention and enables RGB computations to be performed offline, reducing reliance on the CKB chain.
...
#### Global State and Its Security Risks

To boost the contract's capabilities, RGB++ introduces the concept of a global-state cell. The "Shared State and Stateless Contracts" white paper by RGB++ explains this feature:

> Imagine a Cell on CKB that manages shared state across multiple users. This global state is overseen by a stateless contract, meaning changes can be made by anyone who meets the contract's conditions, without needing a specific digital signature provider.
>
> A potential issue with this setup is the risk of the global-state cell being used by other parties, which could render a CKB transaction invalid if the specified global state utxo no longer exists.
>
> To address this, we've introduced the Intent Cell to act as a mediator. Users can deterministically record their intended actions in the Intent Cell, which then works with third-party aggregators to interact with the global state Cell. This process batch calculates the intentions of multiple parties and merges the results onto a standard shadow cell.
...
When elements like the global state are introduced, this changes the dynamics, as the public order of transactions A and B on different paths can influence the outcome. Consider the following scenario:
...
The Intent Cell solution presented in the white paper does not fully address this issue. While it ensures that transactions submitted to CKB are valid, it doesn't guarantee that both A and B will be recorded on the CKB chain promptly. (This is an important consideration for solutions that commit before publishing.) In the original RGB model, as long as the client correctly constructs the state transition and commitment, the contract operates smoothly. However, in RGB++, the validity of the commitment made first on Bitcoin is now controlled by CKB, and incorporating the global-state cell into RGB might not be a secure choice.
...
This pseudocode provides a basic framework for how an RGB execution engine could be constructed within the EVM environment. The BTC_STATE contract simulates the ability to interact with the Bitcoin blockchain to retrieve transaction data, while the RGB_CONTRACT demonstrates the process of committing and executing RGB transactions on the EVM chain.
It's important to note that this is a simplified representation and actual implementation would require thorough testing and security audits to ensure the robustness and safety of the contract. Additionally, the pseudocode assumes the existence of certain functions (indicated by the ellipsis "...") that would need to be fully defined for a complete implementation.